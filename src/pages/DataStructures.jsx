...
export default function DataStructures() {
  ...
  const updateVisuals = (currentTree) => {
    ...
    const traverse = (node, x, y, level) => {
      ...
      const horizontalSpacing = 200 / Math.pow(1.5, level); 
      const verticalSpacing = 70;

      if (node.left) {
        const leftX = x - horizontalSpacing;
        const leftY = y + verticalSpacing;
        lines.push({ id: `line-${node.value}-L`, x1: x, y1: y, x2: leftX, y2: leftY });
      ...
      if (node.right) {
        ...
        lines.push({ id: `line-${node.value}-R`, x1: x, y1: y, x2: rightX, y2: rightY });
  ...
  const handleInsert = (e) => {
    ...
    // Clone the tree instance to force React to re-render
    ...
    <div className="flex flex-col items-center min-h-full bg-slate-950 p-4 sm:p-8 font-sans text-white">
      <div className="w-full max-w-5xl flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Visualize Binary Search Tree (BST) insertions.</p>
        </div>
        ...
        <form onSubmit={handleInsert} className="flex gap-2">
          <input 
            ...
            required
          />
          <button type="submit" className="px-6 py-2 bg-emerald-500 text-white rounded-md hover:bg-emerald-400 transition font-bold shadow-lg shadow-emerald-500/20">
            Insert
          </button>
          <button type="button" onClick={clearTree} className="px-4 py-2 bg-slate-800 rounded-md hover:bg-slate-700 transition font-medium border border-slate-700">
            Clear
          </button>
        </form>
      </div>
